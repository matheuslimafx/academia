<?php
session_start();
?>
<!--INÍCIO DAS MENSAGENS:-->
<div class="resultado_login">
    Resultado
</div>
<!--FIM DAS MENSAGENS-->
<!--FORMULÁRIO DE LOGIN:-->
<body class="login">
    <div class="container">
        <h2>Academia Performance Fit</h2>
        <div class="well col-md-6 fundo-login">
            <form class="pf_form" action="" method="POST">
                <input type="hidden" name="callback" value="verificar_login">
                <div class="form-group">
                    <label>E-mail</label><br>
                    <input type="text" name="email_usuario" class="form-control" placeholder="E-mail">
                </div>
                <div class="form-group">
                    <label>Senha</label><br>
                    <input type="password" name="senha_usuario" class="form-control" placeholder="Senha">
                </div>
                <div class="form-group ">
                    <button type="submit" class="btn btn-info col-md-4" value="Entrar" name="Entrar">Entrar</button>
                </div>
                <div class="form-group">
                    <div class="pf_load">
                        <img alt="" title="" src="Views/img/load.gif"/>
                    </div>
                </div>
            </form>
        </div>
        <div class="form-group col-md-2"></div>
        <div class="form-group col-md-4">
            <img src="img/icon02.png">
            <br><br>
            <label class="descricao">
                <p>Academia Performance Fit</p>
                <p>Telefone: (62) 3277-1457</p>
                <p>Unid. 1 - Rua -75 Qd.195 LT.02 Cid. Vera Cruz 2</p>
                <p>Unid. 2 - Av. Dona Ilda Qd.17 Lt.01 Bairro Ilda</p>
            </label>
        </div>
    </div>   
</body>
<?php
session_destroy();
?>